...
for epoch in range(num_epochs):
    ...
    for i, (text, aspect_label, opinion_label, opinion, sentiment_label, pos) in enumerate(zip(Word_feature[start_idx:end_idx], aspect_sequences[start_idx:end_idx], opinion_sequences[start_idx:end_idx], opinion[start_idx:end_idx], sentiment[start_idx:end_idx], POS_feature[start_idx:end_idx])):
        ...
        output = aspect_tensor.squeeze(1)
        ...
        # 检查长度是否相等
        if len(pred_label) == len(aspect_label):
            pass
        elif len(pred_label) < len(aspect_label):
            aspect_label = aspect_label[-len(pred_label):]
        else:  # len(predict_label) > len(aspect_term_seq)
            aspect_label = [0] * (len(pred_label) - len(aspect_label)) + aspect_label
        #ATE loss
        aspect_loss = log_loss(aspect_label, pred_label)

        # #ATE accuracy
        # pred_label_tensor = torch.tensor(pred_label)
        # label_tensor = torch.tensor(aspect_label)
        # correct = (pred_label_tensor == label_tensor).sum().item()
        # aspect_accuracy = correct / len(aspect_label)


        #OTE Task
        opinion_tensor = transformer_encoder(gcn_tensor)
        output = opinion_tensor.squeeze(1)
        ...
        # OTE loss使用负对数似然作为损失,对数似然损失（Log-Likelihood Loss）
        opinion_loss = log_loss(opinion_label, pred_label)
        ...
        # correct = (pred_label_tensor == label_tensor).sum().item()
        # opinion_accuracy = correct / len(opinion_label)
        ...
        selected_values = output[opinion]
        ...
        #ASC loss 计算损失值
        sentiment_loss = criterion(selected_values, one_hot_labels)

        # #ASC accuracy
        # sentiment_tensor_label = torch.tensor(sentiment_label)  # 标签张量
        # correct = (pred_tensor_sentiment == sentiment_tensor_label).sum().item()
        # sentiment_accuracy = correct / len(one_hot_labels)


        #Triplet loss
        loss = 1/3 * aspect_loss + 1/3 * opinion_loss  + 1/3 *sentiment_loss
        ...
        # accuracy = 1/3 * aspect_accuracy + 1/3 * opinion_accuracy  + 1/3 *sentiment_accuracy
    ...
    with open('TripletTask.txt', 'a') as file:
        output_string = f"Epoch {epoch + 1}/{num_epochs}, Average Loss: {average_loss}\n"
